...
from utils.cache import AuthCache                  # Модуль для кэширования ключа api
import requests
...
class ChatApp:
    ...
    def __init__(self):
        ...
        self.auth = AuthCache()                    # Инициализация системы кэширования ключа
    ...
    def update_balance(self):
        ...
        try:
            ...
            if not self.api_client.api_key:
                self.balance_text.value = "Баланс: нет ключа"
    ...
    def __show_key_input(self, page):
        key_input = ft.TextField(label="API ключ", width=300)
        status = ft.Text("")
        
        def save(e):
            self.api_client.api_key = key_input.value
            self.api_client.headers["Authorization"] = f"Bearer {key_input.value}"
            
            # Проверяем ключ через /auth/key
            try:
                response = requests.get(
                    f"{self.api_client.base_url}/auth/key",
                    headers=self.api_client.headers
                )
                if response.status_code == 200:
                    import random
                    pin = f"{random.randint(0,9999):04d}"
                    self.auth.set(key_input.value, pin)
                    page.clean()
                    page.add(ft.Text(f"Ваш PIN: {pin}", size=20, color="green"))
                    page.update()
                    import time
                    time.sleep(2)
                    page.clean()
        ...
        page.add(ft.Column([
            ft.Text("Введите API ключ OpenRouter", size=20),
            key_input,
            ft.ElevatedButton("Сохранить", on_click=save),
            status
        ], alignment="center"))

    def __show_pin_input(self, page, stored_pin):
        pin_input = ft.TextField(label="Пин-код", password=True, width=200)
        ...
        page.add(ft.Column([
            ft.Text("Введите пин-код", size=20),
            pin_input,
    ...
    def main(self, page: ft.Page):
        ...
        api_key, stored_pin = self.auth.get()

        if not api_key:
            # Нет ключа - показываем поле ввода
            self.__show_key_input(page)
            return
        
        if not hasattr(self, '_pin_verified'):
            # Есть ключ - проверяем пин
            self.__show_pin_input(page, stored_pin)
            return
        

        api_key, _ = self.auth.get()
        self.api_client.api_key = api_key
        self.api_client.headers["Authorization"] = f"Bearer {api_key}"

        # Обновляем баланс т.к. ключ определился
        self.update_balance()
